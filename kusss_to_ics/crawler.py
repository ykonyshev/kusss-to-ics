...
BASE_URL: Final = URL("https://www.kusss.jku.at/kusss/")
COURSE_CATALOG_PAGE: Final = "coursecatalogue-start.action"
COURSES_FOR_CURRICULUM_PAGE: Final = "coursecatalogue-get-segments.action"
COURSE_CLASS_GROUP_PAGE: Final = "coursecatalogue-get-courseclasses.action"
...
class Crawler(AbstractAsyncContextManager):
    ...
    async def crawl_class(self, class_id: str, class_code: str) -> Class:
        url = (BASE_URL / COURSE_CLASS_PAGE).extend_query({
        ...
        for tr in class_trs:
            ...
            if class_href_url.query.get("courseclass") == class_id:
                matching_tr = tr
        ...
        if matching_tr is None:
            raise MissingTagError(
                selector="tbody tr",
                page_url=url,
                context="Should contain the anchor that links to the specific class ID.",
            )
    ...
    async def crawl_course(self, course_classes_id: str) -> Course:
        url = (BASE_URL / COURSE_CLASSES_PAGE).extend_query({
            "courseclass": course_classes_id
        })
        soup = await self.get_soup(url)
    ...
    async def crawl_course_class_group(self, *, curriculum_id: str, course_class_group_id: str) -> Course:
        ...
        anchor_href_url = URL(str(anchor_href))
        course_classes_id = anchor_href_url.query.get("courseclass")
        if course_classes_id is None:
    ...
    async def crawl_curriculum_by_id(self, curriculum_id: str) -> Courses:
        ...
        courses = await async_map_collect(
            lambda course_group_id: self.crawl_course_class_group(
